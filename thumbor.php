...
class Thumbor
{
	...
	// Settings parameter for future implementation
	...
	public function load_hooks()
	{
		...
		add_filter('the_content', array($this, 'webp_picture_fix'), 9999);
	}
		...
		if (!is_null($images)) {
			...
			foreach ($images as $key) {
				...
				if (strpos($images[3][$i], 'size-full') !== false) {
					$replacement = '<picture><source srcset="';
					foreach ($sizes as $key => $size) {
						if ($size['width'] !== '') {
							$url = $this->get_thumbor_image($images[7][$i], $size['width'], $size['height'], false,  array(
								'format' =>  'webp'
							));
							$replacement .= $url . ' ' . $size['width'] . 'w,';
						}
					}
					$replacement .= '" type="image/webp" sizes="(max-width: 1000px) 100vw, 1000px" /><img' . $images[1][$i] . 'class=' . $images[2][$i] . $images[3][$i] . $images[4][$i] . $images[5][$i] . 'src=' . $images[6][$i] . $images[7][$i] . $images[8][$i] . $images[9][$i] . '></picture>';
